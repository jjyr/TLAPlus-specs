...
TypeInvariant ==
    ...
    /\ memQ \in Seq(Proc * MReq) \* memQ is a sequence of <proc, req> pairs.
Coherence ==
...
DoRd(p) ==
    ...
    /\ buf[p].op = "Rd"
    /\ cache[p][buf[p].addr] /= NoVal
    /\ buf' = [buf EXCEPT ![p] = cache[p][buf[p].addr]]
    ...
    LET f[i \in 0.. Len(memQ)] ==
        IF i = 0 THEN wmem
            ELSE IF memQ[i][2].op = "Rd"
                THEN f[i - 1] \* return previous vmem
                ELSE [f[i - 1] EXCEPT ![memQ[i][2].addr] = memQ[i][2].val]
    IN f[Len(memQ)]
MemQWr ==
    LET r == Head(memQ)[2]
    IN  /\ (memQ /= <<>>) /\ (r.op = "Wr")
...
Evict(p, a) ==
    /\ (ctl[p] = "waiting") => (buf[p].adr /= a) \* Can't evict in waiting adr
...
\* Last modified Sun May 23 23:11:39 CST 2021 by jiang
\* Created Sun May 23 21:45:45 CST 2021 by jiang
